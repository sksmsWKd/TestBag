	...
	private void buildDictionaryFromDB() {
		...
		try (Connection con = DriverManager.getConnection(URL, USER, PWD);) { // finally 호출 필요없음.
			...
			while (rs.next()) {
				...
				//rs.getString(1);     -1번칼럼, 위치값
				String eword = rs.getString("eword");
				//rs.getString(2);  -2 번칼럼, 위치값
...
/*
 *  1. DB연결
 * 1-1. jdbc 드라이버 메모리에 적재 (1번만 하면 됨, 생성자에서 이미 함)
 * 1-2. DriverManager.getConnection(url,id,pw) 호출해서
 * Connection 객체를 구성하고
 * 
 *  2. SQL문 실행
 * 2-1.Connection 객체에게 실행할 SQL문을 실행준비 요청, con.prepareStatement(sql);  -> 이 메서드 실행시 문제가 없다면
 * 																			PreparedStatement 객체 반환
 * 2-2. 
 * PreparedStatement.executeUpdate();    -> 실행할 SQL문이 insert delete update 문일때
 ...
 */
		try (Connection con = DriverManager.getConnection(URL, USER, PWD)) { //서버 url, id, pw 명시해야함
			...
			String sql = "insert into dict values(?,?)";  //? 는 place holder 이고 , 실행 준비 시킨 후에 
														//실행 직전에 값을 설정 하고, 실행 요청을 보낸다.
			
			
			con.prepareStatement(sql);
			// sql 문을 db 서버로 보내서 실행준비 시켜라.
			
			PreparedStatement pstmt = con.prepareStatement(sql);
			
			//실행된거 아님 , 실행준비만 되어있다.
						
			//? 자리에 들어갈 칼럼의값 데이터 타입에 따라
			//적절한 setXXXXX메서드를 실행
			...
			pstmt.executeUpdate();//실행요청  -> 실행할 SQL문이 insert delete update 문일때.
